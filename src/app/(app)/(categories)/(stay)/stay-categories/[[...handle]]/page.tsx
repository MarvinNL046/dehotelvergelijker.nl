...
const Page = async ({ 
  ...
  if (location) {
    if (category.handle === 'all') {
      dynamicHeading = (
        <>
          Alle hotels in <span className="text-primary-600">{location}</span>
        </>
      )
    } else {
      dynamicHeading = (
  ...
  let dynamicDescription = (
    <div className="flex items-center sm:text-lg">
      ...
      <span className="ms-2.5">{convertNumbThousand(category.count)} hotels</span>
    </div>
  )
      ...
      <div className="relative container mt-14 lg:mt-24">
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-2.5 gap-y-5">
          <h2 id="heading" className="scroll-mt-20 text-lg font-semibold sm:text-xl">
            Meer dan {convertNumbThousand(category.count)} hotels
            {location ? (
              <>
          ...
          <Button color="white" className="ms-auto" href={'/stay-categories-map/' + category.handle + (location ? `?location=${encodeURIComponent(location)}` : '')}>
