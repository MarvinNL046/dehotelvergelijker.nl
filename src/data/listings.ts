...
//  STAY LISTING  //
export async function getStayListings(): Promise<any[]> {
  try {
    ...
    if (process.env.DATABASE_URL && process.env.NODE_ENV === 'production') {
      ...
      return hotels.map(hotel => ({
        id: `stay-listing://${hotel.id}`,
        date: hotel.createdAt.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
        listingCategory: hotel.stars ? `${hotel.stars} Star Hotel` : 'Hotel',
        title: hotel.name,
        handle: hotel.slug,
        description: hotel.description,
        featuredImage: hotel.images[0] || 'https://images.pexels.com/photos/6129967/pexels-photo-6129967.jpeg?auto=compress&cs=tinysrgb&dpr=3&h=750&w=1260',
        galleryImgs: hotel.images.slice(0, 4),
        like: false,
        address: hotel.address,
        reviewStart: hotel.rating || 4.5,
        reviewCount: hotel.reviewCount,
        price: hotel.minPrice ? `€${hotel.minPrice}` : 'Check availability',
        maxGuests: 4,
        bedrooms: 1,
        bathrooms: 1,
        beds: 2,
        saleOff: hotel.featured ? '-10% today' : null,
        isAds: false,
